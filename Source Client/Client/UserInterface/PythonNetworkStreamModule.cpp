...
PyObject* netSendGayaCraft(PyObject* poSelf, PyObject* poArgs)
{
	int pos;
	if (!PyTuple_GetInteger(poArgs, 0, &pos))
		return Py_BuildException();
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendGayaCraft(pos);
	return Py_BuildNone();
}

PyObject* netSendGayaMarket(PyObject* poSelf, PyObject* poArgs)
{
	int pos;
	if (!PyTuple_GetInteger(poArgs, 0, &pos))
		return Py_BuildException();
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendGayaMarket(pos);
	return Py_BuildNone();
}

PyObject* netSendGayaRefresh(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendGayaRefresh();
	return Py_BuildNone();
}
...
{
	const CPythonNetworkStream::CTIPMANAGER* TipManager = CPythonNetworkStream::Instance().TipManager;
	if (TipManager == nullptr) 
		return Py_BuildValue("s", "");

	const long lMapIndex = CPythonBackground::Instance().GetWarpMapIndex();
...
#ifdef ENABLE_GAYA_SYSTEM
		{ "GayaCraft",								netSendGayaCraft,						METH_VARARGS },
		{ "GayaMarket",								netSendGayaMarket,						METH_VARARGS },
		{ "GayaRefresh",							netSendGayaRefresh,						METH_VARARGS },
#endif
#ifdef ENABLE_AURA_SYSTEM
