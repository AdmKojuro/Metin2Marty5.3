...
bool LoadLocaleData(const char* localePath)
{
...
#ifdef ENABLE_SHINING_SYSTEM
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
	snprintf(szShiningTable, sizeof(szShiningTable), "locale/common/shining_table.txt");
#else
	...
	if (rkNetStream.TipManager == nullptr) {
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		snprintf(szTip, sizeof(szTip), "locale/common/loading_tip_list.txt");
#else
		snprintf(szTip, sizeof(szTip), "%s/loading_tip_list.txt", localePath);
#endif
		if (tempTipManager->InitList(szTip) == false)
		{
		...
		if (tempTipManager->InitVnum(szTip) == false)
		{
			TraceError("LoadLocaleData - LoadTipVnum(%s) Error", szTip);
			return false;
		}

		rkNetStream.TipManager = tempTipManager.release(); // Take Ownership
	}
#endif
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
	snprintf(szAuraScale, sizeof(szAuraScale), "locale/common/aura_scale.txt");
#else
