	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		if (pTitle)
		{
			...
			if (pLevel)
			{
				...
				if (pLanguageInstance)
				{
					int iLevelWidth, iLevelHeight;
					pLevel->GetTextSize(&iLevelWidth, &iLevelHeight);
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - iTitleWidth - iLevelWidth - pLanguageInstance->GetWidth() - 6.0f, pTextTail->y - 10.0f);
				}
#endif
		...
		else
		{
			...
			if (pLevel)
			{
				...
				if (pLanguageInstance)
				{
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - iLevelWidth - pLanguageInstance->GetWidth() - 6.0f, pTextTail->y - 10.0f);
				}
#endif
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
	...
	if (!prLanguage)
	{
		...
		if(pCharacterInstance->IsPC() && bLanguage)
		{
			...
			char szFileName[256];
			sprintf(szFileName, "icon/flag/%s.tga", langName.c_str());
