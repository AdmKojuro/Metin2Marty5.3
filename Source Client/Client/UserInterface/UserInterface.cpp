...
bool PackInitialize(const char * c_pszFolder)
{
	...
	CEterPackManager::Instance().RegisterPack ("pack/pc3", "*");
	CEterPackManager::Instance().RegisterPack ("pack/property", "*");
	CEterPackManager::Instance().RegisterPack ("pack/root", "*");
	...
	CEterPackManager::Instance().RegisterPack ("pack/metin2_patch_gem", "*");
	...
	CEterPackManager::Instance().RegisterPack ("pack/metin2_patch_aura_system", "*");
	CEterPackManager::Instance().RegisterPack ("pack/metin2_patch_hide_costume", "*");
	...
	CEterPackManager::Instance().RegisterPack ("pack/metin2_patch_event", "*");
	CEterPackManager::Instance().RegisterPack ("pack/zeron_darealpain", "*");
	...
	CMemoryTextFileLoader sura;
	sura.Bind(file.Size(), pvData);

	const std::string& strPackType = sura.GetLineString(0);
	...
	#ifdef ENABLE_FAST_CARREGAMENTO
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	#ifdef BLOCK_CHANGE_NAME_BIN
	WIN32_FILE_ATTRIBUTE_DATA wfad;
	if (!GetFileAttributesEx ("metin2client.bin", GetFileExInfoStandard, &wfad) && !GetFileAttributesEx ("metin2client.exe", GetFileExInfoStandard, &wfad))
	{
		std::string szFileName;
		GetExcutedFileName(szFileName);
		szFileName = CFileNameHelper::NoPath(szFileName);
		CFileNameHelper::ChangeDosPath(szFileName);

		char szName[1024];
		snprintf(szName, sizeof (szName), "Prohibido cambiar el nombre del archivo metin2client por %s.", szFileName.c_str());
		MessageBox(NULL, szName, ApplicationStringTable_GetStringz(IDS_APP_NAME, "APP_NAME"), MB_ICONSTOP);
		ShellExecute (0, "open", "Launchermetin2client.exe", 0, 0, SW_SHOWNORMAL);
