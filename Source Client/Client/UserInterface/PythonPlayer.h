...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	...
	private:
		...
		std::vector<TItemData2> m_AuraItemInstanceVector;
	...
	public:
		...
		void				SetAuraItemData(BYTE bSlotIndex, const TItemData2& rItemInstance);
		void				DelAuraItemData(BYTE bSlotIndex);

		BYTE				FineMoveAuraItemSlot();
		BYTE				GetAuraCurrentItemSlotCount();

		BOOL				GetAuraItemDataPtr(BYTE bSlotIndex, TItemData2** ppInstance);
	...
	public:
		...
		void	SendDayNightUpdate(char* mode);
#endif
