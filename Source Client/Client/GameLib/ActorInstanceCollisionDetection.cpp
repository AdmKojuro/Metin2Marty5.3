...
BOOL CActorInstance::TestActorCollision(CActorInstance & rVictim)
{
	...
	if (rVictim.GetRace()>=34001 && rVictim.GetRace()<=34099) // Mob Vnum
		return false;
#endif

#ifdef REMOVE_SHOPS_COLLISION
	if (rVictim.GetRace()>=30000 && rVictim.GetRace()<=30001) // Mob Vnum
		return false;
#endif

#ifdef REMOVE_MOUNTS_COLLISION
	if (rVictim.GetRace()>=20101 && rVictim.GetRace()<=20299) // Mob Vnum
		return false;
#endif
