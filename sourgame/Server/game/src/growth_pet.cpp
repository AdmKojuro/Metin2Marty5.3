...
CGrowthPet::CGrowthPet(LPCHARACTER pOwner)
{
	...
	m_pFeatherSkillEvent = nullptr;
	m_pSummonItem = nullptr;
...
LPGROWTH_PET CGrowthPet::Summon(LPITEM pSummonItem, bool bIsResummon /* = false */)
{
	...
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "%s is currently fast asleep.", m_strName.c_str());
		return nullptr;
	}
...
void CGrowthPet::Unsummon()
{
	...
	if (m_pOwner && m_pOwner->GetActiveGrowthPet())
		m_pOwner->SetActiveGrowthPet(nullptr);
...
bool CGrowthPet::Update()
{
	...
	// Trigger auto skill events if the pet has them
...
/*
	Trigger FollowCharacter function in case pet is too far away
	from it's owner or victim if the feather skill is activated.
*/
bool CGrowthPet::UpdateFollowAI()
{
	...
	// Check if feather skill is active and set follow target based on vicim
	LPCHARACTER pTarget = m_pFeatherSkillEvent ? m_pOwner->GetVictim() : m_pOwner;

	// There is no victim, return to normal state and follow the owner
	if (!pTarget)
		pTarget = m_pOwner;
...
/*
	Makes the pet follow it's owner in normal state. If feather skill
	is activated, pet will follow owner's target instead.
*/
...
/*
	Change pet points. If bAmount is set to true, previous points will be 
	discarded. If bPacket is set to true, user's client will be updated with new 
	information.
*/
void CGrowthPet::ChangePetPoint(BYTE bIdx, int iVal, bool bAmount /* false */, bool bPacket /* = true */)
{
	switch (bIdx)
	{
...
/*
	Sends pet information to trader's companion used in item tooltip.
*/
...
/*
	Send a level step effect on each 25% of mob experience. 
*/
...
/*
	Send a packet to user updating skill information used in skill tooltip.
*/
void CGrowthPet::UpdateSkillPacket()
{
	...
	for (int i = 0; i < PET_SKILL_COUNT_MAX; ++i)
	{
		...
		if (m_aSkill[i].bSkill)
		{
			...
			{
				// Formula1/Formula3 is used by auto skills only (Activation PCT & Duration)
...
void CGrowthPet::NameChange(const char* c_szName, LPITEM pChangeNameItem, LPITEM pUpBringingItem)
{
	...
	// Refresh the name if the pet was summoned
	if (IsSummoned())
...
/*
	Handles feed packet in 3 different forms. Lifetime increase,
	evolution incrase and item EXP reward. Returns a packet to user
	based on result.
*/
void CGrowthPet::Feed(const TPacketCGPetFeed* feedPacket)
{
	...
	switch (feedPacket->index)
	{
		...
		case FEED_EXP_EVENT:
		{
			...
			m_pOwner->ChatPacket(CHAT_TYPE_INFO, "Experience has increased to %d points.", dwTotalEXP);
		} break;
...
/*
	Pet can only receive exp if it is lower level than owner
	and it's evolution stage allows it.
*/
...
/*
	Pet can level up if it is lower level than owner
	and it's evolution stage allows it.
*/
...
/*
	Increase pet's experience, either battle or item type.
	If both experiences are full, pet will level up. Return
	if pet is higher level than owner or evolution is too low.
*/
bool CGrowthPet::RewardEXP(BYTE bType, DWORD dwEXP)
{
	...
	switch (bType)
	{
		case EXP_TYPE_MOB:
		{
			...
			// If both exp types are full, proceed to level up
...
/*
	This function is used when pet has to be manually leveled-up
	with the owner because it was unable to do so due restrictions.
	Return true if pet has leveled-up.
*/
bool CGrowthPet::LevelUp()
{
	...
	if (dwCurrentMobEXP >= GetRequiredMobExp(dwPetLevel) &&
		dwCurrentItemEXP >= GetRequiredItemExp(dwPetLevel))
	{
		// Reset EXP values
		ChangePetPoint(POINT_UPBRINGING_PET_EXP, 0, true);
		ChangePetPoint(POINT_UPBRINGING_PET_ITEM_EXP, 0, true);
...
/*
	Increase pet's lifetime based on type. Eggs/Upbringings
	increase pet's life by 3~5%, Protein Snack by 100%.
*/
...
/*
	If revive conditions are justified, pet will be revived.
	In the opposite situation, send a failure packet.
*/
void CGrowthPet::PremiumFeed(const TPacketCGPetRevive* revivePacket, LPITEM pSummonItem)
{
	...
	// Remove required number of items
	for (int i = 0; i < PET_REVIVE_MATERIAL_SLOT_MAX; ++i)
	{
		LPITEM item = m_pOwner->GetInventoryItem(revivePacket->pos[i]);
		DWORD dwItemCount = item->GetCount();	// Lets not rely on packet item count
...
/*
	Remind the player pet can be evolved if it meets the requirements.
*/
...
/*
	Function will increase pet's evolution if user has enough
	evolve materials and pet meets the conditions. Returns false
	on critical error.
*/
bool CGrowthPet::Evolve(const TPacketCGPetFeed* feedPacket)
{
	...
	if (itemVec.size() == PET_EVOL_MAX_ITEM_COUNT)
	{
		...
		// Change pet skin on the last evolution by resummoning the pet
		if (GetPetPoint(POINT_UPBRINGING_PET_EVOL_LEVEL) == PET_EVOLUTION_4)
			Summon(m_pSummonItem, true);
		
		if (GetPetPoint(POINT_UPBRINGING_PET_EVOL_LEVEL) == PET_EVOLUTION_2 ||
			GetPetPoint(POINT_UPBRINGING_PET_EVOL_LEVEL) == PET_EVOLUTION_3)
		{
			// Reset EXP values
			ChangePetPoint(POINT_UPBRINGING_PET_EXP, 0, true);
			ChangePetPoint(POINT_UPBRINGING_PET_ITEM_EXP, 0, true);
...
/*
	Used by command for manual evolution. Unlike normal 
	evolve function, this one will resummon pet in any case.
*/
...
/*
	Refresh affects given by battle points and passive skills.
*/
...
/*
	Increase battle points. Triggered on every level up. If it is an
	attribute change method, do not update player's client values which
	will be updated later on in AttrChange function.
*/
void CGrowthPet::IncreaseBattlePoints(DWORD dwLevel, bool bAttrChange /* = false */)
{
	...
	// Do not refresh affect for attr-change as pet is not summoned
	if(!bAttrChange)
		RefreshAffect();
}

/*
	Sends a packet to player with pet's type.
*/
...
/*
	If attr change is a success, change pet's type & battle points.
	Send a packet updating interface at the end.
*/
void CGrowthPet::AttrChange(LPITEM pSummonItem, LPITEM pAttrChangeItem)
{
	...
	// Mini Monkey & Mini Spider cannot achieve types 7/8
...
void CGrowthPet::LearnSkill(BYTE bSlot, LPITEM pSkillBookItem)
{
	...
	// Skill has been learned already
	for (const auto& skill : m_aSkill)
	{
...
/*
	Activate passive or auto skill.
*/
bool CGrowthPet::UseSkill(BYTE bSkillAffect)
{
	...
	// There is no skill conneceted to the affect
	int iSkillSlot = GetSlotBySkill(bSkillAffect);
	if (iSkillSlot == -1)
		return false;

	// Skill cooltime check
	...
	else if (pSkillProto->GetSkillType() == PET_SKILL_USE_TYPE_AUTO)
	{
		...
		// Activate % for affect removal skill has to be hard-coded as there is no sufficient formula.
		...
		// Set skill cooltime even if the skill activation fails
...
void CGrowthPetManager::DeleteGrowthPet(DWORD dwID, bool bDestroy /* = false */)
{
	auto it = m_growthPetGlobalMap.find(dwID);
