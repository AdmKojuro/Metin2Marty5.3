...
EVENTFUNC(shutdown_event)
{
	...
	else
	{
		...
		SendNotice(LC_TEXT("¼Ë´Ù¿îÀÌ %dÃÊ ³²¾Ò½À´Ï´Ù."), *pSec);
#else
		char buf[64];
...
#ifdef __MULTI_LANGUAGE_SYSTEM__
	SendNotice(LC_TEXT("%dÃÊ ÈÄ °ÔÀÓÀÌ ¼Ë´Ù¿î µË´Ï´Ù."), iSec);
#else
	char buf[64];
...
ACMD(do_set_name_shop)
{
	...
	if (m_stShopSign.length()<1) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("???O?i©©? ???i¡Æ¢§ ?¡ÀCO¥ì? ¡íoA¢§ ??¢¬¡×?¢¬¡¤I ¡íoA¢§?¡í ?¡© ?o ??¢©?¢¥?¢¥?."));
		return;
	}
...
ACMD(do_bio)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÀÓ¹«¸¦ ºÎ¿©¹ÞÀ¸·Á¸é ·¹º§ 30ÀÌ µÇ¾î¾ß ÇÕ´Ï´Ù."));
		return;
	}
	...
	if (vecArgs[1] == "mission")
	{
		...
		if (count < bio_data[level][2])
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÀÓ¹«¸¦ ¿Ï·áÇÏ±â¿¡´Â Àç·á°¡ ÃæºÐÇÏÁö ¾Ê½À´Ï´Ù."));
				return;
			}
			...
			if (isUnutkanlik)
			{
				...
				else
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("´ë±â ½Ã°£ÀÌ Áö³ªÁö ¾ÊÀ¸¸é »óÇ°À» ¹è¼ÛÇÒ ¼ö ¾ø½À´Ï´Ù. ¿¤¸¯¼­¸¦ »ç¿ëÇÏ¸é ½Ã°£À» ÁÙÀÏ ¼ö ÀÖ½À´Ï´Ù."));
						return;
					}
				}
			}
			...
			if (prob >= number(1, 100))
			{
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°³Ã¼ÀÇ Ç°ÁúÀÌ ÁÁ°í ±àÁ¤ÀûÀÎ °á°ú¸¦ ¾ò¾ú½À´Ï´Ù."));
			}
			else
			{
				time = get_global_time() + bio_data[level][3];
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¹°°ÇÀÇ »óÅÂ°¡ ÁÁÁö ¾Ê¾Ò½À´Ï´Ù. ´õ °¡Á®¿À¼¼¿ä."));
			}
		...
		if (bio_data[level][5] != 0)
		{
			if (count == bio_data[level][2])
			{
				if (ch->CountSpecifyItem(bio_data[level][5]) < 1)
				{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("½Å¼ºÇÑ ¹°°ÇÀ» °¡Á®¿ÀÁö ¾ÊÀ¸¸é ÀÓ¹«¸¦ ¿Ï·áÇÒ ¼ö ¾ø½À´Ï´Ù."));
					return;
				}
				else
				{
					...
					if (bio_data[level][14] == 0)
					{
						...
						if (bio_data[level][6] != 0)
						{
							...
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Ä³¸¯ÅÍ¿¡ È¿°ú°¡ ¼º°øÀûÀ¸·Î Ã³¸®µÇ¾ú½À´Ï´Ù."));
						...
						if (newLevel >= bio_max)
						{
							...
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¸ðµç ÀÓ¹«¸¦ ¿Ï·áÇß½À´Ï´Ù. »õ·Î¿î ¹Ì¼ÇÀ¸·Î °ð ºË±æ ¹Ù¶ø´Ï´Ù."));
...
//#ifdef TEXTS_IMPROVEMENT
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÃÖ´ë 10ÃÊ¸¶´Ù ÀÌ ÀÛ¾÷À» ¼öÇàÇÒ ¼ö ÀÖ½À´Ï´Ù."));
...
//#ifdef TEXTS_IMPROVEMENT
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Áö±ÝÀº ÅÚ·¹Æ÷Æ®ÇÒ ¼ö ¾ø½À´Ï´Ù."));
...
//#ifdef TEXTS_IMPROVEMENT
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾çÀÌ ºÎÁ·ÇØ¿ä."));
