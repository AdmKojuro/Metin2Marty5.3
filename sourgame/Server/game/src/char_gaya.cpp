...
void CHARACTER::loadGaya()
{
	...
	while (fgets(one_line, 256, fp))
	{
		...
		else TOKEN("TIME_REFRESH"){load_gaya_values.time_refresh = value1;}
	}
	fclose(fp);
}
...
void CHARACTER::InfoGayaMarker()
{
	...
	ChatPacket(CHAT_TYPE_COMMAND, "GayaMarketTime %d",GetGayaState("system_gaya.gaya_time_world_4") - get_global_time());
...
EVENTFUNC(check_time_market_event)
{
	...
	if (ch->GetGayaState("system_gaya.gaya_time_world_4") - get_global_time() <= 0)
	{
		ch->SetGayaState("system_gaya.gaya_time_world_4", get_global_time() +  ch->GetTimeRefresh());
...
void CHARACTER::StartCheckTimeMarket()
{
	...
	GayaUpdateTime = event_create(check_time_market_event, info, PASSES_PER_SEC(3));	// 1ºÐ
}