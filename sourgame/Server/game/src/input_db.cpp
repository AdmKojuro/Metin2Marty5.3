...
EVENTFUNC(fix_shop_event)
{
	...
	if (pRes->uiNumRows>0)
	{
		...
		{
			DWORD id,pid,vnum,count,socket[6],attr[7][2],m_dwRefineElement;
			int col = 0;
			...
			char query[1024];
			sprintf(query, "INSERT INTO player_gift SET owner_id=%d,vnum=%d,count=%d,refine_element=%d,reason='%s',`from`=replace(\"%s\",' ','_'),status='WAIT',date_add=NOW()",
				pid, vnum,count, m_dwRefineElement, LC_TEXT("SHOP_REASON"), row[3]);
