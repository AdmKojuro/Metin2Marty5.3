...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (pinfo->type == CHAT_TYPE_SHOUT)
	{
		...
		if (g_bGlobalShout)
		{
...
#ifdef __MULTI_LANGUAGE_SYSTEM__
				sprintf(chatbuf_global, "|L%s|l %s %s", LC_LOCALE(ch->GetDesc()->GetLanguage()), staff_color.c_str(), chatbuf);
#else
			...
			{
#ifdef __FLAG_IMAGE_SHOUT_LINE__
				std::string kingdom_red = "|Eflag/shinsoo|e";
#else
				...
				if (g_bDetailShout)
#ifdef __MULTI_LANGUAGE_SYSTEM__
					sprintf(chatbuf_global, "|L%s|l [CH%d] Lv.%d %s %s", LC_LOCALE(ch->GetDesc()->GetLanguage()), char_channel, char_level, kingdom_red.c_str(), chatbuf);
#else
					sprintf(chatbuf_global, "[CH%d] Lv.%d %s %s", char_channel, char_level, kingdom_red.c_str(), chatbuf);
#endif
				else
#ifdef __MULTI_LANGUAGE_SYSTEM__
					sprintf(chatbuf_global, "|L%s|l %s %s", LC_LOCALE(ch->GetDesc()->GetLanguage()), kingdom_red.c_str(), chatbuf);
#else
			...
			{
#ifdef __FLAG_IMAGE_SHOUT_LINE__
				std::string kingdom_yel = "|Eflag/chunjo|e";
#else
				...
				if (g_bDetailShout)
#ifdef __MULTI_LANGUAGE_SYSTEM__
					sprintf(chatbuf_global, "|L%s|l [CH%d] Lv.%d %s %s", LC_LOCALE(ch->GetDesc()->GetLanguage()), char_channel, char_level, kingdom_yel.c_str(), chatbuf);
#else
					sprintf(chatbuf_global, "[CH%d] Lv.%d %s %s", char_channel, char_level, kingdom_yel.c_str(), chatbuf);
#endif
				else
#ifdef __MULTI_LANGUAGE_SYSTEM__
					sprintf(chatbuf_global, "|L%s|l %s %s", LC_LOCALE(ch->GetDesc()->GetLanguage()), kingdom_yel.c_str(), chatbuf);
#else
			...
			{
#ifdef __FLAG_IMAGE_SHOUT_LINE__
				std::string kingdom_blue = "|Eflag/jinno|e";
#else
				...
				if (g_bDetailShout)
#ifdef __MULTI_LANGUAGE_SYSTEM__
					sprintf(chatbuf_global, "|L%s|l [CH%d] Lv.%d %s %s", LC_LOCALE(ch->GetDesc()->GetLanguage()), char_channel, char_level, kingdom_blue.c_str(), chatbuf);
#else
					sprintf(chatbuf_global, "[CH%d] Lv.%d %s %s", char_channel, char_level, kingdom_blue.c_str(), chatbuf);
#endif
				else
#ifdef __MULTI_LANGUAGE_SYSTEM__
					sprintf(chatbuf_global, "|L%s|l %s %s", LC_LOCALE(ch->GetDesc()->GetLanguage()), kingdom_blue.c_str(), chatbuf);
#else
...
void CInputMain::ItemDrop(LPCHARACTER ch, const char * data)
{
	...
	if(pinfo->gold >0 && pinfo->cheque >0)
		ch->ChatPacket(CHAT_TYPE_INFO, "No puedes tirar mas de 2 monedas.");
...
#ifdef __ENABLE_CHEQUE_SYSTEM__
	if (pinfo->gold >0 && pinfo->cheque >0)
		ch->ChatPacket(CHAT_TYPE_INFO, "No puedes tirar m? de 2 monedas.");
	else if (pinfo->cheque > 0)
		ch->DropCheque(pinfo->cheque);
	else if (pinfo->gold > 0)
		ch->DropGold(pinfo->gold);
...
#ifdef __ENABLE_CHEQUE_SYSTEM__
		case EXCHANGE_SUBHEADER_CG_CHEQUE_ADD:	// arg1 == amount of gold
			if (ch->GetExchange())
			{
...
#ifdef ENABLE_SEARCH_ITEM_DROP_ON_MOB
void CInputMain::DropItemInfoSend(LPCHARACTER ch, const char* c_pData)
{
	struct packet_send_drop_item_system * pinfo = (struct packet_send_drop_item_system *) c_pData;
