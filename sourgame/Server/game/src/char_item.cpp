					...
					case USE_SPECIAL:
						...
						switch (item->GetVnum())
						{
							...
							case 30344:
								{
									char QueryWorld[512];
									snprintf(QueryWorld, sizeof(QueryWorld), "SELECT activacion FROM player.offline_shop_decoration WHERE id = %u LIMIT 1", GetPlayerID());
									std::auto_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery(QueryWorld));
									if (pmsg->Get()->uiNumRows == 0) {
										char QueryWorld1[512];
										snprintf(QueryWorld1, sizeof(QueryWorld1), "INSERT INTO player.offline_shop_decoration(id , activacion) VALUES(%u,%d)",GetPlayerID(),1);
										DBManager::Instance().DirectQuery(QueryWorld1);

										// item->SetCount(item->GetCount()-1);
										ChatPacket(CHAT_TYPE_INFO, "[Paquete de cachemira] Ahora puedes darle un aspecto personalizado a tu tienda privada.");
									}
									else{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Paquete de cachemira] Ya esta activo, puedes elegir el aspecto de tu tienda desde el panel de creacion de la tienda privada."));
										return false;
									}
								}
								break;
							...
							case 38060:
								{
										...
										if(CountEmotion() >= 12){
											ChatPacket(CHAT_TYPE_INFO,"[EmotionExtra] No puede poner mas emociones.");
											return false;
										}
										...
										if (pct <= 70){
											InsertEmotion();
											ChatPacket(CHAT_TYPE_INFO,"[EmotionExtra] Emocion agregada con exito.");
										}

									#else
										ChatPacket(CHAT_TYPE_INFO,"[EmotionExtra] Sistema Sin Funcionamiento.");
									#endif
...
{
	//return false; // Funci? desactivada.
	...
	if (!CanHandleItem())
		return false;
	LPITEM item = ITEM_MANAGER::instance().CreateItem(3, cheque);
	if (item)
	{
...
bool CHARACTER::DestroyItem(TItemPos Cell)
{
	...
	ChatPacket(CHAT_TYPE_INFO, "El articulo %s fue destruido.", item->GetName());
	ITEM_MANAGER::instance().RemoveItem(item);
...
#ifdef __ENABLE_CHEQUE_SYSTEM__
void CHARACTER::GiveCheque(int iAmount) // No hay soporte de cheque para grupos.
{
	if (iAmount <= 0)
		return;
		...
		if (item->IsOwnership(this))
		{
			...
			else
			{
				...
				else if (item->IsSkillBook())
				{
					...
					if (iOpenCount < 3)
					{
						ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s was added to your special inventory"), item->GetName());
...
void CHARACTER::GiveSkillBook(DWORD dwSkillVnum, WORD wCount)
{
	for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
	{
		...
		{
			WORD wCount2 = MIN(g_bItemCountLimit - pkItem->GetCount(), wCount);
			wCount -= wCount2;
...
bool CHARACTER::DoRefineElement(BYTE bArg)
{
	...
	DWORD dwRefineCost = 10000000; // Default value in case some shit happen
	int iSuccessProb = 0;
