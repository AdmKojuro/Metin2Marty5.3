...
bool CShopManager::CreateOfflineShop(LPCHARACTER owner, const char *szSign, const std::vector<TShopItemTable *> pTable, DWORD id)
{
	...
	sprintf(szQuery, "insert into player_shop set item_count=%d,player_id=%d,name=\"%s\",map_index=%d,x=%ld,y=%ld,z=%ld,ip='%s',date=NOW(),date_close=FROM_UNIXTIME(%d),channel=%d,npc=%d,npc_decoration=%d",
		pTable.size(), owner->GetPlayerID(), szSign, map_index, x, y, z, inet_ntoa(owner->GetDesc()->GetAddr().sin_addr), date_close, g_bChannel, owner->GetOfflineDecoration(), owner->GetOfflineDecoration1());
#else
...
bool CShopManager::StartOfflineShop(DWORD id, bool onboot)
{
	...
	char mDecoration[50];
	//Shop Decoration
	SQLMsg * pkMsg(DBManager::instance().DirectQuery("SELECT player_shop.name,player_shop.player_id,player.name as player_name,player_shop.map_index,player_shop.npc,player_shop.npc_decoration,player_shop.x,player_shop.y,player_shop.z,UNIX_TIMESTAMP(player_shop.date_close),player_shop.id from player_shop left join player on player.id=player_shop.player_id where player_shop.id='%d'", id));
#else
...
#ifdef ENABLE_OFFSHOP_DECO
			//Shop Decoration
			str_to_number(mNpc, row[4]);
			snprintf(mDecoration, sizeof(mDecoration), "%s", row[5]);
			//Shop Decoration
			str_to_number(x, row[6]);
			str_to_number(y, row[7]);
			str_to_number(z, row[8]);
			str_to_number(time, row[9]);
#else
...
#ifdef ENABLE_OFFSHOP_DECO
	LPCHARACTER ch = CHARACTER_MANAGER::Instance().SpawnMob(mNpc, map_index, x, y, z, true, 0, false);
	if (ch)
	{

		std::string m_variable00 = mDecoration;
		std::string m_variable11 = m_variable00+name;
		//Shop Decoration
		...
		ch->OpenShop(id, m_variable11.c_str(), onboot);
		...
		if (owner){
			...
			if (owner->GetOfflineDecoration() != 30000 || owner->GetOfflineDecoration1() != 0){
				DBManager::instance().DirectQuery("DELETE FROM player.offline_shop_decoration where id = %u", owner->GetPlayerID());
				if (owner->GetOfflineDecoration1() != 0){
					//m_npc_decoration = 0;
					owner->OfflineDecoration1(0);
				}
				if (owner->GetOfflineDecoration() != 0){
					//m_npc_offline = 30000;
					owner->OfflineDecoration(30000);
				}
			}
		}
		return true;
...
{
	LPSHOP pkShop = FindPCShop(ch->IsPrivShop() && ch->GetRaceNum() == 30000 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30001 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30002 ? ch->GetPrivShop() : ch->GetVID()
		|| ch->GetRaceNum() == 30003 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30004 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30005 ? ch->GetPrivShop() : ch->GetVID() 
		|| ch->GetRaceNum() == 30006 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30007 ? ch->GetPrivShop() : ch->GetVID());
	...
	m_map_pkShopByPC.erase(ch->IsPrivShop() && ch->GetRaceNum() == 30000 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30001 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30002 ? ch->GetPrivShop() : ch->GetVID()
		|| ch->GetRaceNum() == 30003 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30004 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30005 ? ch->GetPrivShop() : ch->GetVID() 
		|| ch->GetRaceNum() == 30006 ? ch->GetPrivShop() : ch->GetVID() || ch->GetRaceNum() == 30007 ? ch->GetPrivShop() : ch->GetVID());
	M2_DELETE(pkShop);
}
