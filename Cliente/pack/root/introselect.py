...
class MyCharacters:
	...
	def LoadCharacterData(self):
		...
		for i in xrange(CHARACTER_SLOT_COUNT_MAX):
			...
			if last_playtime <= 0:
				...
				self.myUnitDic[PrepareLastPlay] = self.MyUnit(i, name, level, race, playtime, guildname, form, hair, acce, stat_str, stat_dex, stat_hth, stat_int, change_name) #last_playtimeself.myUnitDic[i] = self.MyUnit(i, name, level, race, playtime, guildname, form, hair, stat_str, stat_dex, stat_hth, stat_int, change_name) #last_playtime
			else:
				self.SetPriorityData(last_playtime)
				self.myUnitDic[last_playtime] = self.MyUnit(i, name, level, race, playtime, guildname, form, hair, acce, stat_str, stat_dex, stat_hth, stat_int, change_name) #last_playtime
		...
		for index, data in enumerate(self.PriorityData):
			...
			self.MakeCharacter(index, DestDataDic["NAME"], DestDataDic["RACE"], DestDataDic["FORM"], DestDataDic["HAIR"], DestDataDic["ACCE"])
	...
	#def MakeCharacter(self, slot, name, race, form, hair):
		try:
			...
			import exception
			exception.Abort("MakeCharacter.MakeCharacter.MakeCharacter")
...
#		self.NameList.append(getChild("name_wolfman"))
	...
	def OnUpdate(self):
		...
		if self.SelectEmpire and constInfo.RESELECT_EMPIRE == 0:
			self.SelectEmpire = False
