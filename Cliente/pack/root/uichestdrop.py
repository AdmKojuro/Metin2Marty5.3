...
class ChestDropWindow(ui.ScriptWindow):
	...
	def SetOpenAmount(self, amount):
		...
		self.openChestButtonSingle.SetText("Abrir %d cofre(s)" % self.openAmount)
		self.openChestButtonMultiple.SetText("Abrir %d cofre(s)" % self.openAmount)
	...
	def RefreshItems(self, chestVnum):
		...
		if not self.chestDrop.has_key(self.currentChest):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Este objeto no tiene nada adentro. Reporta el problema con un administrador.")
			self.Close()
			return
