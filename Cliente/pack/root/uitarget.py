...
class TargetBoard(ui.ThinBoard):
	...
	if app.ENABLE_SEND_TARGET_INFO:
		class InfoBoard(ui.ThinBoard):
			...
			def __init__(self):
				...
				self.itemScrollBar.SetPosition(0, 0)
				...
				if race in constInfo.MONSTER_INFO_DATA:
					if len(constInfo.MONSTER_INFO_DATA[race]["items"]) == 0:
						...
						if len(constInfo.MONSTER_INFO_DATA[race]["items"]) > itemListBox.GetViewItemCount():
							#import chat
							#chat.AppendChat(1,"count items true {}".format(len(constInfo.MONSTER_INFO_DATA[race]["items"])))
							self.itemScrollBar.SetPosition(itemListBox.GetRight(), itemListBox.GetTop())
	...
	def ResetTargetBoard(self):
		...
		if app.ENABLE_SEND_TARGET_INFO:
			self.infoButton.Hide()
			self.infoButton.showWnd.Close()
		if app.ENABLE_ELEMENT_ADD and self.elementImg:
			self.elementImg = None
		self.SetSize(250, 40)
	...
	if app.ENABLE_VIEW_TARGET_DECIMAL_HP:
		def SetHP(self, hpPercentage, iMinHP, iMaxHP):
			if constInfo.ENABLE_FIX_TARGET_BOARD:
					...
					if app.ENABLE_VIEW_TARGET_PLAYER_HP:
						...
						if chr.GetInstanceType(self.vid) == chr.INSTANCE_TYPE_PLAYER:
							if showingButtonCount == 8:
								self.SetSize(max(150 + 75 * 3, 8 * 75), self.GetHeight())
							if showingButtonCount == 7:
								self.SetSize(max(150 + 75 * 3, 7 * 75), self.GetHeight())
							if showingButtonCount == 6:
								self.SetSize(max(150 + 75 * 3, 6 * 75), self.GetHeight())
							if showingButtonCount == 5:
								self.SetSize(max(150 + 75 * 3, 5 * 75), self.GetHeight())
							if showingButtonCount == 4:
								self.SetSize(max(150 + 75 * 3, 4 * 75), self.GetHeight())
							if showingButtonCount == 3:
								self.SetSize(max(150 + 75 * 3, 3 * 75), self.GetHeight())
							if showingButtonCount == 2:
								self.SetSize(max(150 + 75 * 3, 2 * 75), self.GetHeight())
							if showingButtonCount == 1:
								self.SetSize(max(150 + 75 * 3, 1 * 75), self.GetHeight())
	...
	if app.ENABLE_HIDE_COSTUME_SYSTEM:
		def SetCostumeHide(self, costumeHide):	
			...
			self.toolTip.AppendTextLine("Aura: %s"%(self.GetInfoHide(aura)))
	...
	def RefreshButton(self):
		...
		if app.ENABLE_HIDE_COSTUME_SYSTEM:
			self.HideCostume.SetPosition(self.nameLength + 32, 14)
			self.HideCostume.SetWindowHorizontalAlignCenter()
	...
	if app.ENABLE_ELEMENT_ADD:
		def SetElementImage(self, element):
			if element > 0 and element in self.ELEMENT_IMG_PATH.keys():
				self.elementImg = ui.ExpandedImageBox()
				self.elementImg.SetParent(self)
				self.elementImg.LoadImage(self.ELEMENT_IMG_PATH[element])
				self.elementImg.SetPosition(0, 0)
				if localeInfo.IsARABIC():
					self.elementImg.SetWindowHorizontalAlignRight()
				else:
					self.elementImg.SetPosition(-40, 0)
					self.elementImg.SetWindowHorizontalAlignLeft()
