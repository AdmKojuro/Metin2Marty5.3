...
class WhisperDialog(ui.ScriptWindow):
	...
	def LoadDialog(self):
		...
		if app.ENABLE_GOOGLE_TRANSLATE_INGAME:
			self.chatLine.OnKeyDown = self.ChatLineOnKeyDown
			self.chatLine.OnKeyUp = self.ChatLineOnKeyUp
		...
		if app.ENABLE_MULTI_LANGUAGE_SYSTEM:
			...
			self.countryFlag.SetPosition(150, 12)
	...
	if app.ENABLE_GOOGLE_TRANSLATE_INGAME:
		def ChatLineOnKeyDown(self, key):
			if texttranslator_v4.wnd.OnKeyDownEvent(key):
				return True
			return ui.EditLine.OnKeyDown(self.chatLine, key)

		def ChatLineOnKeyUp(self, key):
			if texttranslator_v4.wnd.OnKeyUpEvent(key):
				return True
			return ui.EditLine.OnKeyUp(self.chatLine, key)
